...
class TestAzureAuthStatus:
    """Tests for the azure_auth_status tool."""

    @pytest.mark.asyncio
    async def test_auth_status_success(self):
        """Test azure_auth_status returns authentication status."""
        with patch.object(server.cloud, "get_auth_status", new_callable=AsyncMock) as mock_auth:
            ...
            result = await server.azure_auth_status()
    ...
    async def test_auth_status_handles_exception(self):
        """Test azure_auth_status handles exceptions gracefully."""
        with patch.object(server.cloud, "get_auth_status", new_callable=AsyncMock) as mock_auth:
...
class TestAzureAccountShow:
    """Tests for the azure_account_show tool."""

    @pytest.mark.asyncio
    async def test_account_show_success(self):
        """Test azure_account_show returns account information."""
        with patch.object(server.cloud, "get_account_info", new_callable=AsyncMock) as mock_account:
            ...
            result = await server.azure_account_show()
    ...
    async def test_account_show_handles_exception(self):
        """Test azure_account_show handles exceptions gracefully."""
        with patch.object(server.cloud, "get_account_info", new_callable=AsyncMock) as mock_account:
...
class TestAzureAccountList:
    """Tests for the azure_account_list tool."""

    @pytest.mark.asyncio
    async def test_account_list_success(self):
        """Test azure_account_list returns subscription list."""
        with patch.object(server.cloud, "list_subscriptions", new_callable=AsyncMock) as mock_list:
            ...
            result = await server.azure_account_list()

            assert "Error" not in result
            mock_list.assert_called_once()

    @pytest.mark.asyncio
    async def test_account_list_handles_exception(self):
        """Test azure_account_list handles exceptions gracefully."""
        with patch.object(server.cloud, "list_subscriptions", new_callable=AsyncMock) as mock_list:
...
class TestAzureAccountListLocations:
    """Tests for the azure_account_list_locations tool."""

    @pytest.mark.asyncio
    async def test_account_list_locations_success(self):
        """Test azure_account_list_locations returns locations."""
        with patch.object(server.cloud, "list_locations", new_callable=AsyncMock) as mock_list:
            mock_list.return_value = "Azure Locations:\n- eastus\n- westus\n- westeurope"
            result = await server.azure_account_list_locations()

            assert "Error" not in result
            mock_list.assert_called_once()

    @pytest.mark.asyncio
    async def test_account_list_locations_handles_exception(self):
        """Test azure_account_list_locations handles exceptions gracefully."""
        with patch.object(server.cloud, "list_locations", new_callable=AsyncMock) as mock_list:
...
class TestAzureTenantList:
    """Tests for the azure_tenant_list tool."""

    @pytest.mark.asyncio
    async def test_tenant_list_success(self):
        """Test azure_tenant_list returns tenant information."""
        with patch.object(server.cloud, "get_tenant_info", new_callable=AsyncMock) as mock_tenant:
            ...
            result = await server.azure_tenant_list()

            assert "Error" not in result
            mock_tenant.assert_called_once()

    @pytest.mark.asyncio
    async def test_tenant_list_handles_exception(self):
        """Test azure_tenant_list handles exceptions gracefully."""
        with patch.object(server.cloud, "get_tenant_info", new_callable=AsyncMock) as mock_tenant:
